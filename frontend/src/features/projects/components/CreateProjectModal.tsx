...
export function CreateProjectModal({ open, onClose, onCreated }: CreateProjectModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = res.data.data as { projectId?: number; project_id?: number };
      const projectId = data.projectId ?? data.project_id;
      if (projectId == null) {
        setError("Tạo dự án thành công nhưng không nhận được ID. Tải lại trang danh sách.");
        return;
      }
      setName("");
      setDescription("");
      ...
      setError("Không tạo được dự án. Kiểm tra quyền hoặc thử lại sau.");
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <header className="flex items-center justify-between border-b border-slate-100 bg-gradient-to-r from-slate-50 to-blue-50/50 px-6 py-4">
          <section className="flex items-center gap-3">
            ...
            <section>
              ...
              <p className="text-xs text-slate-500">Bạn sẽ là Quản trị viên của dự án</p>
            </section>
          </section>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-6 py-5">
          <section className="space-y-1.5">
            ...
            <Input
              ...
              placeholder="VD: Website Task Manager"
              value={name}
          ...
          <section className="space-y-1.5">
            ...
            <textarea
              ...
              placeholder="Mục tiêu, phạm vi hoặc ghi chú cho team..."
              value={description}
